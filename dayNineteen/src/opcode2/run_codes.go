...
var opCodeMap = map[string]OpCodeFunction{
	"muli": Muli,
	"borr": Borr,
	"gtri": Gtri,
	"eqri": Eqri,
...
func GetFunctionByName(name string) OpCodeFunction {
	result, exists := opCodeMap[name]
	if !exists {
		panic("Function with name" + name + " does not exist")
	}
	return result
}

type Operation struct {
	OpInstruction string
	First         int64
	Second        int64
	Output        int64
}
